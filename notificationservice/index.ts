...
async function poll(db: Db) {
  ...
  while (true) {
    ...
    for (const request of requests) {
      log.debug(`Checking weather for ${request.email} request ${request.location}`)
      log.debug(`request:  ${request}`)

      if (checkNotifiedToday(request)) {
        log.debug(`Notified today for ${request.email} request ${request.location}`)
        continue;
      }
    ...
    log.info('******************  Finished polling for requests  **********************')
