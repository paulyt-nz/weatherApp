...
export async function getRequests(
    db: Db
): Promise<WeatherNotificationSubscription[]> {
...
export function checkWeatherMatchesConstraints(
    ...
    if (constraints.windSpeed) {
        if (constraints.windSpeed.min !== undefined) {
            if (weather.windSpeed < constraints.windSpeed.min) {
                log.debug(
                    `windspeed is less than min - min=${constraints.windSpeed.min} actual=${weather.windSpeed}`
                );
                return false;
            }
        }
        if (constraints.windSpeed.max !== undefined) {
            if (weather.windSpeed > constraints.windSpeed.max) {
                log.debug(
                    `windspeed is more than max - max=${constraints.windSpeed.max} actual=${weather.windSpeed}`
                );
                return false;
            }
        }
    ...
    if (constraints.temperature) {
        if (constraints.temperature.min !== undefined) {
            if (weather.temperature < constraints.temperature.min) {
                log.debug(
                    `temerature is less than min - min=${constraints.temperature.min} actual=${weather.temperature}`
                );
                return false;
            }
        }
        if (constraints.temperature.max !== undefined) {
            if (weather.temperature > constraints.temperature.max) {
                log.debug(
                    `temperature is more than max - max=${constraints.temperature.max} actual=${weather.temperature}`
                );
                return false;
            }
        }
    ...
    if (constraints.humidity) {
        if (constraints.humidity.min !== undefined) {
            if (weather.humidity < constraints.humidity.min) {
                log.debug(
                    `humidity is less than min - min=${constraints.humidity.min} actual=${weather.humidity}`
                );
                return false;
            }
        }
        if (constraints.humidity.max !== undefined) {
            if (weather.humidity > constraints.humidity.max) {
                log.debug(
                    `humidity is more than max - max=${constraints.humidity.max} actual=${weather.humidity}`
                );
                return false;
            }
        }
    ...
    if (constraints.windDir !== undefined) {
        let windDirMatch = constraints.windDir.some(
            (dir) => dir === weather.windDir
        ); // true or false

        if (windDirMatch === false) {
            log.debug(
                `wind direction does not match - expected=${constraints.windDir} actual=${weather.windDir}`
            );
            return false;
        }
    }
