...
export function createNotification(
    ...
    log.debug(
        `Creating notification for ${request.email} request ${request.location}`
    );
...
export async function sendNotification(
    ...
    const mailjet = Mailjet.apiConnect(
        MailjetApikeyPublic as string,
        MailjetApikeyPrivate as string,
        { config: {}, options: {} }
    );
...
export function checkNotifiedToday(
    request: WeatherNotificationSubscription
): boolean {
