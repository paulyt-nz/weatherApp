...
export default function MainApp() {
    function showSuccessMessage() {
        toast.success(
            <div>
                ...
                <br />
                We will email you when the weather is right!
                <br />
                ☀️🌈😎
            </div>,
            {
    ...
    const [shownConstraints, setShownConstraints] = useState<ShownContraints>(
        initialShownConstraints
    );
    const [inputConstraints, setInputConstraints] = useState<InputContraints>(
        inititialInputConstraints
    );
    ...
    function handleCheckboxChange(clickedCheckbox: keyof ShownContraints) {
        setShownConstraints((prevState) => {
            ...
            if (checkedCount === 1 && prevState[clickedCheckbox]) {
                showErrorMessage(
                    "At least one constraint must remain selected."
                );
                return prevState;
            }
    ...
    function handleInputChange(
        currentInput: keyof InputContraints,
        value: string
    ) {
    ...
    function handleWindDirInputChange(e: React.ChangeEvent<HTMLInputElement>) {
        ...
        setInputConstraints((prevConstraints) => {
            ...
            if (e.target.checked) {
                ...
                updatedWindDirs = prevConstraints.windDirInput.filter(
                    (dir) => dir !== value
                );
            }
    ...
    function checkConstraints(request: WeatherNotificationSubscription) {
        if (shownConstraints.showWindDir) {
            if (
                !request.constraints.windDir ||
                !request.constraints.windDir.length
            ) {
                throw new Error(
                    "No wind direction selected! Either add a wind direction or deselect that option in the checkbox."
                );
            }
        }
        ...
        if (shownConstraints.showWindSpeed) {
            if (
                !request.constraints.windSpeed ||
                !request.constraints.windSpeed.min
            ) {
                throw new Error(
                    "Missing minimum wind speed! Either add a minimum windspeed or deselect that option in the checkbox."
                );
            }
            if (!request.constraints.windSpeed.max) {
                throw new Error(
                    "Missing maximum wind speed! Either add a maiximum windspeed or deselect that option in the checkbox."
                );
            }
            if (
                Number(request.constraints.windSpeed.min) >
                Number(request.constraints.windSpeed.max)
            ) {
                throw new Error(
                    "Minimum wind speed must be less than maximum wind speed!"
                );
            }
        }
        ...
        if (shownConstraints.showTemp) {
            if (
                ...
                !request.constraints.temperature.min
            ) {
                throw new Error(
                    "Missing minimum temperature! Either add a minimum temperature or deselect that option in the checkbox."
                );
            }
            if (!request.constraints.temperature.max) {
                throw new Error(
                    "Missing maximum temperature! Either add a maximum temperature or deselect that option in the checkbox."
                );
            }
            if (
                Number(request.constraints.temperature.min) >
                Number(request.constraints.temperature.max)
            ) {
                throw new Error(
        ...
        if (shownConstraints.showHumidity) {
            if (!inputConstraints.humidityMinInput) {
                throw new Error(
                    "Missing minimum humidity! Either add a minimum humidity or deselect that option in the checkbox."
                );
            }
            if (!inputConstraints.humidityMaxInput) {
                throw new Error(
                    "Missing maximum humidity! Either add a maximum humidity or deselect that option in the checkbox."
                );
            }
            if (
                Number(inputConstraints.humidityMinInput) >
                Number(inputConstraints.humidityMaxInput)
            ) {
                throw new Error(
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            coords = await getCoordsFromLocation(
                inputConstraints.locationInput
            );
    ...
    return (
        <div
            className="bg-gray-200 min-h-screen flex flex-col min-h-screen bg-cover bg-center"
